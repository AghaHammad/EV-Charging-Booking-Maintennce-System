...
export default function SavedMethodCard({
  ...
  const iconWrapperClasses = joinClasses(
    ...
    <div className="rounded-[22px] border border-white/6 bg-[linear-gradient(180deg,rgba(14,37,50,0.96),rgba(11,30,42,0.98))] px-4 py-5 transition hover:border-[#12dfff]/12">
      <div className="flex items-center justify-between gap-4">
        ...
        >
          <div className={iconWrapperClasses}>
            {visual.icon ? (
              ...
              <span className={visual.labelClasses}>{visual.label}</span>
            )}
          </div>
        ...
        <div className="flex shrink-0 items-center gap-2">
          {isPrimary ? (
            <span className="rounded-xl border border-white/10 px-3 py-1.5 text-[12px] text-white/52">
              Primary
            </span>
          ) : (
            <button
